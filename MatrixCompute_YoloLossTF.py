...
def square_wh(boxes):
  ...
  if len(boxes.get_shape().as_list()) == 4:
...
def computeYoloLossTF( pred_classes, pred_conf, pred_boxes, gt_conf, gt_classes, ind_obj_i, gt_boxes_j0):
  ...
  ious_j0 = 1#compute_ious( squared_pred_boxes_j0 , squared_gt_boxes_j0 )
  #pbs_j1 = [pb for pb in tf.split(0, 49, squared_pred_boxes_j1 )] # BREAK INTO chunks of 4
  ious_j1 = 1#compute_ious( squared_pred_boxes_j1 , squared_gt_boxes_j0 )
  ...
  obj_ious = 0 #final_ious * gt_conf[:,0]
  gt_conf_j0 = 0# tf.where(j0_mask, obj_ious, tf.zeros_like(pred_conf_j0))
  gt_conf_j1 = 0# tf.where(j1_mask, obj_ious, tf.zeros_like(pred_conf_j0))
  ...
  noobj_ious = 0#final_ious * gt_conf_noobj[:,0]
  gt_conf_j0_noobj = 0#tf.where(j0_mask_noobj, noobj_ious, tf.zeros_like(pred_conf_j0) )
  gt_conf_j1_noobj = 0#tf.where(j1_mask_noobj, noobj_ious, tf.zeros_like(pred_conf_j0) )
  ...
  class_loss = 0#gt_classes - pred_classes # both are 49 x 20
  class_loss = tf.square(class_loss)
  class_loss = 0#tf.reduce_sum(class_loss, reduction_indices=[1]) # along all classes
