...
def getData(getPickledData,tlImagesPklFilename):
	"""
	INPUTS:
	-	N/A
	OUTPUTS:
	...
	"""
	if getPickledData:
		with open(tlImagesPklFilename, 'rb') as f:
			annotatedImages = pickle.load(f)
	else:
		annotatedImages = pre_process_data(tl_data_path)
		with open(tlImagesPklFilename, 'wb') as f:
...
def sampleMinibatch(annotatedImages, plot_yolo_grid_cells, plot_bbox_centerpoints):
	...
	for i, bbox in enumerate(annotatedImage.BoundingBoxes):
		x_cent = bbox.x_min[1] + bbox.w[3] / 2.
		y_cent = bbox.y_min[2] + bbox.h[4] / 2.
		normalizedXCent, normalizedYCent, gridCellRow, gridCellCol = normXYToGrid(x_cent,y_cent,im)
		normalizedW = bbox.w[3] * 1.0 / im.shape[1] # dividing by im width
		normalizedH = bbox.h[4] * 1.0 / im.shape[0] # dividing by im height
		gridCellRow = int(gridCellRow)
		gridCellCol = int(gridCellCol)
		classIdx = bbox.category[0] # convert string to int
